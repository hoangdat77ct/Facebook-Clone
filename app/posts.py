...
def upload_file():
    ...
    for file in files:
        if file and allowed_file(file.filename):
            ...
            filename = str(val)+file.filename
            hash_string = hashlib.sha256(filename.encode()).hexdigest()
            filename_str = secure_filename(hash_string+format_img[1])
            file.save(os.path.join(config.UPLOAD_FOLDER, filename_str))
            success = True
            list_files.append(filename_str)
        else:
